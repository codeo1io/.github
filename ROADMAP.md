...
- signals: research 2026-09-21: host gh is 2.83.2 (2025-12-10) vs latest v2.101.0 (~16 minors behind; nothing currently broken); cycle-6 2026-09-25 re-verified live: upstream v2.101.0 (published 2026-09-15), host still 2.83.2 — every REST probe this cycle succeeded, so drift remains convenience-grade with no API breakage
...
- id: `rm-019` | track: reliability | priority: 92.0 | status: done in-tree, ship-gated (cycle-5 E1 2026-09-24: exactly ONE mirror implementation — the hardened scripts/sync_data_branch.py — with the twin retired; copy-set parity restored: BOTH kanban OWNERS references (t_acd6a2e8 + t_851e7951) and control-plane/corrections.yaml mirrored from the canonical main-tree copy, so origin/data's stale pre-B3 corrections claim heals at the first post-landing 08:40 run; parity + hardening locked by retargeted tests/test_control_plane_shims.py; README Layout/Automation + AGENTS.md 'Control-plane data mirror' section document the pipeline. SHIP-GATED: heals in production at the first 08:40 run after the commit gate. Prior history preserved — cycle-5 2026-09-23 assess: signals inverted in reality; see below) — REOPENED (cycle-6 2026-09-25 assess 8995c5f981e, live evidence: copy-set parity is FALSE in production — origin/data control-plane/cron-inventory.json == '{}' since the first post-landing 08:40 run (9f94762 2026-09-24; populated at a18fbc9 09-22 and 9d0500c 09-23) while ~/.hermes/cron/jobs.json holds 57 jobs; root cause scripts/sync_data_branch.py:175 iterates the dict-shaped source as a list (top-level {"jobs":[...]}), so every entry fails the isinstance(job, dict) path and the inventory writes empty; tests/test_control_plane_shims.py:93 _fake_home writes NO jobs.json (zero refs in the suite) so parity + idempotence tests certified the empty write — the fixture never shaped production input; ALSO the single-mirror claim is incomplete: orphan scripts/build_control_plane.py (a third divergent implementation: list-format inventory output, no flock/timeouts, CWD-dependent, 0 callers fleet-wide) still ships in the tree; fix scope: parse both dict/list shapes defensively, production-shaped jobs.json fixture, populated-inventory assertion, remove or hard-redirect the orphan)
...
- signals: research C4 — gh api repos/codeo1io/magic-hermes/rulesets returns HTTP 200 [] on a personal-account public repo (classic protection is 403 plan-gated per constraints #5997/#6021; rulesets are not); richer semantics available (bypass actors, push rules, non-FF) than classic protection; cycle-6 2026-09-25 research 343d872c771f re-verified live on THIS repo: gh api /repos/codeo1io/.github/rulesets -> HTTP 200 [] (not 403) — the surface is implement-ready today, scope stays the public opt-in set (private repos remain plan-gated per constraints #5997/#6021)
...
- signals: research C10 — 3 public repos (.github, magic-hermes, openai-embedding-proxy); OSSF Scorecard is free for public repos and produces evidence-based hardening signals that feed later roadmap cycles; cycle-5 research 2026-09-23: ossf/scorecard-action v2.4.4 (2026-07-23) verified current upstream, free + no-auth for public repos — mechanism ready when prioritized; cycle-6 2026-09-25: ossf/scorecard-action v2.4.4 re-verified latest upstream (published 2026-02-02) — still no upgrade work exists
...
- signals: research RC-4 2026-09-22, extends rm-019 — standing drift facts are one-off evidence strings (host gh 2.83.2 vs upstream v2.101.0 = 19 releases; runner gitleaks vs v8.30.1; action pins vs latest); control-plane-sync.sh already mirrors inventory-shaped data to origin/data; a bounded versions record makes drift self-evidencing daily; cycle-6 2026-09-25 research 343d872c771f: runner version is one REST call (gh api /repos/codeo1io/magic-hermes/actions/runners -> magic-hermes-runner status=online version=2.337.0) — add self-hosted runner versions to the record; upstream currency re-verified this cycle: all pins current (gitleaks v8.30.1 2026-03-21, gitleaks-action v3.0.0 2026-05-30, actions/checkout v7.0.1 2026-07-20, dorny/paths-filter v4.0.3 2026-08-05, scorecard-action v2.4.4 2026-02-02 — zero drift, no upgrade work)
...
<!-- cycle-6 roadmap extension (2026-09-25, conductor run 12a4e0ab6833415b8f2b0b035d14ae17 attempt 311390169930466dbcb8b296a43cc0b9; items rm-043..rm-047 sourced from cycle-6 assess attempt 8995c5f981e44300b358ff865de24399 + research attempt 343d872c771f4992ba18391ae79c1134; reopens rm-019 + rm-037 with live evidence; sanctioned ledger extension per render-marker contract) -->

### Sentinel config-distribution trust chain (caller refs + config identity)
- id: `rm-043` | track: reliability | priority: 88.0 | status: candidate
- signals: cycle-6 assess + research — fleet callers reference the reusable SECURITY-CONTROL workflow at mutable refs: magic-hermes + hermes-agent `uses: codeo1io/.github/.github/workflows/private-leak-sentinel.yaml@main` (any push to main silently changes what they execute; gh api contents of both callers verified 2026-09-25), hermes-gpt `@a86c8ef` (SHA but pre-cycle-4 stale); carried rm-036 rider: private-leak-sentinel.yaml:43 config-ref default d04d86d ≠ current hardened gitleaks.toml (cf3a0fe→659c726 lineage) — @main callers pick up workflow-file changes automatically, so a default bump reaches 2/3 callers with zero caller edits (deployment-speed argument for bumping it in the same batch); gitleaks.toml declares no regexTarget anywhere (pinned 8.30.1 README L366-395 documents secret|match|line semantics — value-suppression intent is currently implicit)
- acceptance: (1) config-ref default bumped to the merged HEAD SHA with a parity test asserting default-ref's gitleaks.toml == HEAD's (fails on drift, catches the next stale default at CI); (2) caller pin policy documented (SHA refs for security controls) and the three callers refreshed to pinned SHAs; (3) value-suppression allowlists made explicit with regexTarget=secret/match; (4) a weekly or cron monitor reports each caller's ref style + effective config identity (stale-SHA detection — the @a86c8ef case today)
- evidence: workflow file shows the new default SHA; parity test green and demonstrably FAILS on a planted stale default; three caller YAMLs show SHA refs via gh api contents; monitor output (run log or archived report) listing per-caller state

### Data-branch content integrity monitor
- id: `rm-044` | track: reliability | priority: 90.0 | status: candidate
- signals: cycle-6 assess P1 — the empty-mirror regression ({} cron-inventory since 9f94762, 57 jobs in source) was UNDETECTABLE in-repo: verify_deployed_artifacts.py stops at deployed HOST artifacts (rm-042's perimeter) and NOTHING asserts data-branch CONTENT; the copy-set claims in README/SECURITY/AGENTS were false for a day before assess caught it — the same class as rm-017's "tests certify the unexecuted artifact", one layer out
- acceptance: the verification perimeter extends to origin/data content: cron-inventory populated exactly when the source jobs.json is non-empty; every mirrored file within its documented bound (watchdog ≤ CONTROL_PLANE_WATCHDOG_MAX_LINES); corrections.yaml at parity with the canonical main-tree copy; violations produce a nonzero/report line; locked by tests with planted violations (empty-inventory-with-nonempty-source, oversized mirror, corrections divergence)
- evidence: planted-violation tests red→green in the suite; a live verification run showing content checks green AFTER the rm-019 fix lands (ship-gated ordering: fix first, monitor second)

### Cycle-6 robustness residuals (mirror branch/index hygiene + protection_drift coverage)
- id: `rm-045` | track: reliability | priority: 68.0 | status: candidate
- signals: cycle-6 assess — sync_data_branch.py:200/216 success paths `git checkout MAIN_BRANCH` regardless of entry branch (a manual run from a non-main checkout strands it on main with rc=0; restore is failure-only today); :86 _restore restores the branch but NOT the index (failure between add and commit leaves staged control-plane files on the restored branch; untested); :109 fetch_ok assigned never used (a failed fetch + stale origin/data ref proceeds to reset --hard); sync_repo_settings.py:159 protection_drift never diffs restrictions and compares only required_pull_request_reviews PRESENCE — desired null + live restrictions (or divergent rpr contents) is silent, no --apply PUT (tests/test_sync_repo_settings.py:174 fixture carries null restrictions with no divergent case)
- acceptance: success path restores the entry branch (or the asymmetry is asserted deliberately + documented); _restore resets the index on the failure path; fetch_ok removed or enforced; protection_drift compares restrictions presence+contents and rpr contents, with a divergent-restrictions fixture proving detection and the apply-mode PUT
- evidence: extended tests for each clause (branch-restore-on-success, dirty-index restore, divergent-restrictions drift); live --dry-run against a fixture with restrictions drift reports the row

### Merge-commit title/message drift-set extension
- id: `rm-046` | track: reliability | priority: 62.0 | status: candidate
- signals: cycle-6 research 343d872c771f — REST fields verified live on user repos (gh api /repos/codeo1io/.github: merge_commit_title=MERGE_MESSAGE, squash_merge_commit_title=MERGE_MESSAGE, squash_merge_commit_message=COMMIT_MESSAGES — all platform defaults, 2026-09-25) while common-settings.yaml manages only the allow_*merge booleans; fleet commit-message policy is unmanaged surface
- acceptance: merge_commit_title / merge_commit_message / squash_merge_commit_title / squash_merge_commit_message added to the managed drift set behind explicit desired values; unit fixtures cover divergent live state; --dry-run reports and --apply PATCHes exactly once per repo
- evidence: unit test with divergent fields reports drift and issues one PATCH; live fleet-wide dry-run shows the new fields evaluated with zero unintended drift

### SARIF → Security-tab visibility trial (verify-first)
- id: `rm-047` | track: customer-experience | priority: 55.0 | status: candidate (verify-first)
- signals: cycle-6 research 343d872c771f — code-scanning API is reachable + empty on this user-owned PUBLIC repo (GET /repos/codeo1io/.github/code-scanning/analyses -> 404 "no analysis found", not 403 permission-gated); gitleaks-action v3.0.0 already emits gitleaks-results.sarif as an action-native artifact (rm-036's E4 verification) but findings are invisible in the Security tab — artifact retention only; public-repo code scanning is free tier
- acceptance: ONE verify-first trial on codeo1io/.github (SHA-pinned upload step or one-shot manual upload); promote to fleet convention only if a User-account public repo accepts the SARIF; otherwise record the exact blocker (PAT security_events scope / account state) as evidence and close rejected
- evidence: Security tab lists analyses for .github post-upload, or a documented failure transcript with the precise API error; decision recorded in corrections.yaml if rejected

## Cycle-6 roadmap note (2026-09-25, post-assess, post-research)

Assess (attempt 8995c5f981e, ce-review report-only, 11 findings at 659c726 ==
origin/main; 55/55 tests green, live dry-run drift_found=0, known-hosts rc=0,
verify_deployed_artifacts rc=1) and research (attempt 343d872c771f, ce-ideate
inline, 7 ranked survivors + 5 evidence-backed rejections, all sources fetched
live 2026-09-25) fed this addendum. STATUS FLIPS: rm-019 REOPENED — the cycle-5
promotion's copy-set parity claim is false in production (cron-inventory mirrors
{} since the first post-landing run; dict-vs-list parsing bug at
sync_data_branch.py:175; test fixture never shaped production input; plus the
orphan build_control_plane.py still shipping); rm-037 REOPENED — the 'dist/'
path entry is substring-unanchored (live A/B: flagged at src/, silent at xdist/),
same escape class as the fixtures-path exemption the cycle-5 narrowing closed.
NEW items: rm-043 (sentinel config-distribution trust chain — caller ref policy
+ the carried rm-036 config-ref default bump + parity test + regexTarget
explicitness + weekly caller-drift monitor), rm-044 (data-branch content
integrity monitor — the rm-042 class extended to mirror content; would have
caught the rm-019 regression), rm-045 (robustness residuals: branch-restore-
on-success, _restore index gap, dead fetch_ok, protection_drift restrictions/
rpr coverage), rm-046 (merge-commit title/message drift-set — live-verified
fields), rm-047 (SARIF→Security-tab verify-first trial). Evidence appended:
rm-015 (main/data divergence after the cap: 12,686 vs 5,000 lines, stale
main-side inventory, single-consumer fact), rm-029 (rulesets API 200 on this
user-owned public repo — implement-ready), rm-036 (rider still unexecuted:
default d04d86d vs cf3a0fe), rm-042 (live rc=1 re-verification), rm-013 (gh
2.83.2 vs 2.101.0 re-verified, no breakage), rm-040 (runner 2.337.0 + all-pin
currency re-check: zero drift), rm-035 (scorecard-action v2.4.4 still latest).
ID-MAPPING CORRECTION: research summary's "runner versions → rm-039" shorthand
resolved against the ledger — versions.json is rm-040 (rm-039 is the Dependabot
item); this addendum records the correct mapping. Rejected this cycle with
rationale: all upstream pin upgrades (zero drift — five pins re-verified with
release dates); adopting competing settings-as-code (safe-settings moved to
github-community-projects but still active-and-org-bound, terraform-provider-
github, peribolos — all org/app/state-bound vs a User-account PAT fleet);
magic-hermes scheduled scanning (EXISTS — daily 03:23 full-history cron in its
caller, candidate rejected on evidence); native secret-scanning replacement
(carried rejection — no custom generic patterns on free user accounts).
Next-cycle candidates in priority order: rm-019 fix batch (dict-shape parsing +
production-shaped fixture + populated-inventory assertion + orphan removal —
unblocks every other mirror claim), rm-037 dist/-anchoring + anchoring-
invariant test, rm-044 content monitor (ships after the rm-019 fix so it
asserts a healthy mirror), rm-043 trust chain (default bump + parity test
reach 2/3 callers instantly via @main; caller SHA refresh follows), rm-022
(solutions-lint JUDGMENT scoping — implement-ready, carried), rm-028 + rm-029
(workflow-permissions pair; rm-029 now implement-ready), rm-045, rm-046,
rm-047 trial, rm-042 host half (owner-side pins), rm-039/rm-034 (Dependabot
family), rm-035. Owner-gated unchanged: rm-015 (menu A/B/C — option B cheapest
given the single-consumer fact), rm-016/rm-027 one-repo enablement +
revert-capture experiment, rm-005/rm-033 host-side pass, gh upgrade (rm-013).

*End of cycle-6 addendum.*
