...
- id: `rm-005` | track: reliability | priority: 96.0 | status: candidate — rc-propagation half landed live 2026-09-25 (cycle-7 F3: wrapper initializes rc=1, captures the sync's exit code, and `exit "$rc"` — unconditional exit 0 gone; host-key ALERT stays report-only by design; wrapper re-pinned in deployed MANIFEST, verify rc=0, propagation proved with a live dry-run tail; RESIDUALS: README rc-contract sentence + a PATH-shimmed failure-mode proof recorded at the commit gate)
...
- acceptance: wrapper exits nonzero when the --apply sync or check_known_hosts fails (report-only VISIBILITY DRIFT lines still OK); log gains a one-line RESULT summary per run; README documents the rc contract; AMENDED cycle-7 F3 (2026-09-25, review finding 6): the host-key ALERT is report-only BY DESIGN and never drives rc — nonzero applies to the sync rc path; the README rc-contract sentence lands with the commit-gate rider
...
- id: `rm-045` | track: reliability | priority: 88.0 | status: done (cycle-7 batch F — LANDED: commit cbf7bf1 merged to origin/main 43d8097, 2026-09-24T21:22Z; cycle-8 assess 2026-09-26 live-verified at HEAD — 68/68 local suite matching README + all ten subfixes present; in-tree history 2026-09-25: all ten subfixes landed in run-8606647750f7 worktree uncommitted — (a) P2 rc semantics: wrapper-present+manifest-missing AND manifest-present+wrapper-missing both grade drift rc=1, rc=2 skip only when both absent, plus the auxiliary-wrapper contract (every deployed *.sh delegating into this repo must pin its delegates; foreign-repo delegations out of scope — the live fleet probe that caught the over-match is lesson L32); (b) staging allowlisted to the run's copy-set; (c) entry-branch restore on success and no-op; (d) deque-bounded mirror tail; (e) drifted as a set — unique-repo count; (f) CHECK_KNOWN_HOSTS env override; (g) AGENTS.md/SECURITY.md docs/SECRETS.md dangling refs replaced; (h) claims.yaml MIRRORED (parity decision: same class as corrections.yaml) with locking test; (i) deploy-router.yaml top-level permissions: contents: read; (j) scripts/build_control_plane.py deleted. +11 locking tests, 66/66 suite green at implement (review re-run confirmed), README count 55→68 after the review-fix pass, no dangling SECRETS refs, grep-clean of build_control_plane. Validation: targeted 48/48 local, full = work-order full_command VERBATIM -> ephemeral-pr ok:true (PR #11 self-scan SUCCESS, refs auto-deleted) + local 66/66; digest validation:v1:eba4ed22…dba2 at implement (base 659c726, 13 surfaces), refreshed post-review-fix to validation:v1:efb1fa85…d936 (docs-truth fixes + 2 textual-discovery edge tests, suite 68). Ship-gate PASSED at the cycle-7 merge; full_tests ephemeral PR #12 self-cleaned; landing digest validation:v1:efb1fa85…d936)
...
## Cycle-7 compound note (2026-09-25, pre-review evidence: implement + targeted/full test outcomes)

Batch F 'truth of executed paths II' implemented end to end in the run worktree
(attempt c44bb0aa116a, ce-work inline): F1 a–j per rm-045 above, F2 = the rm-042 host
pins (MANIFEST 14→19, planted-divergence proof), F3 stretch = the rm-005 wrapper rc
propagation. Test outcomes consumed, not re-run: targeted 48/48 over the four impacted
files (+2 solutions-lint neighbor), full 66/66 locally AND the work-order full_command
VERBATIM -> ephemeral-pr ok:true (PR #11, leak-sentinel-self SUCCESS, conductor/ci-*
refs self-deleted, worktree untouched — see L31). Fresh digest at implement:
validation:v1:eba4ed22db25ee8502fbe3ff2e223c488559cec116f4e4c45005227da217dba2
(base 659c726, 13 executable surfaces; re-derived byte-identical at targeted_tests and
full_tests).
REVIEW-FIX PASS (2026-09-25, attempt 90e0cd66b918): all 3 P2 docs-truth fixes + 3
advisories applied (AGENTS/README pinned-state + copy-set parity + restore-symmetry
prose; rm-005 acceptance descope amended in-line; 2 textual-discovery edge tests —
comment-mention and name-collision, both fail-closed). Digest re-derived post-fix from
the AUTHORITATIVE module: validation:v1:efb1fa85d35da70f1d914776054bd81b923102b03b7831ca2c697247b66ad936
(supersedes eba4ed22 above; suite now 68 tests collected, README exact, targeted 16/16
on the verifier suite). STATUS FLIPS THIS PHASE: rm-045 → done in-tree, ship-gated; rm-042 repo
half closed + host pins landed (final re-pin rider at the commit gate); rm-019 orphan
gap closed; rm-005 rc-propagation half landed live. Lessons appended: L32–L35
(docs/lessons.md). POST-LANDING RIDER (must run at/after the commit gate): re-pin the
three delegate digests in ~/.hermes/scripts/MANIFEST.sha256 once canonical resets, then
verify rc=0 — that closes rm-042 fully. Next-cycle candidates in priority order:
rm-043 probe (owner-gated), rm-044 gate adoption (zizmor v1.30.1/actionlint v1.7.12) —
now joined by the full_tests finding that the cloud gate executes leak-scan only (no
remote pytest): a pull_request-triggered test workflow is the CI-side companion to
rm-044, rm-022 (JUDGMENT scoping), rm-028+029 (permissions pair), the rm-016/rm-027
one-repo probes. Carried unchanged: rm-036 config-ref bump, magic-hermes caller
refresh, rm-033, rm-039 keep-current.

*End of cycle-7 addendum.*

<!-- cycle-8 roadmap extension (2026-09-26, conductor run 7ef4ad0ae4b441f391bd8869b09b35a7 attempt 29fec10530a94077a37e857c7f0ccccc; items rm-046..rm-051 sourced from cycle-8 assess attempt fbba2dfbf5b5 + research attempt d505e465665146edb433bd289669fc11; sanctioned ledger extension per render-marker contract) -->

### Fix the empty cron-inventory mirror (dict-shaped jobs.json) + fixture + canary
- id: `rm-046` | track: reliability | priority: 96.0 | status: done in-tree, ship-gated (cycle-8 batch G1, implement attempt ba17f3a9 at base 43d8097: clauses 1-3 in-tree and verified — dict-tolerant parse payload.get("jobs", payload), '# N jobs' count header, loud rc=1 canary on missing/shapeless/empty, both-shape fixture (the #10848 gap closed), 5 locking tests, suite 68→77, targeted 35/35, full_command ephemeral-pr ok:true PR #13, digest 79a1d3fc; SHIP GATE: the first post-landing daily run must mirror a NON-EMPTY inventory with '# 57 jobs' + the true job ids on origin/data — then flip to plain done)
- signals: cycle-8 assess LIVE 2026-09-26 — scripts/sync_data_branch.py:199 iterates json.loads(jobs.json) as a top-level LIST, but host ~/.hermes/cron/jobs.json is a dict {"jobs":[…],"updated_at":…} (57 jobs): dict iteration yields string keys, the isinstance(job, dict) guard drops them all, so origin/data:control-plane/cron-inventory.json has been `{}` on every daily run since 9f94762 (2026-09-24; live-verified copy {} generated 2026-09-25T08:40:13Z) — the cycle-6 "until fixed" belief (#10829) is FALSE: git log --all -S 'get("jobs"' is empty, no commit ever unwrapped ["jobs"]; the 68-test suite stays green because tests/test_control_plane_shims.py _fake_home creates no jobs.json (lesson #10848 fixture gap persists); AGENTS.md "cron + runners inventories" and SECURITY.md's mirrored-inventory claim are contradicted in production
...
- evidence: post-landing git show origin/data:control-plane/cron-inventory.json (count-bearing header + 57 jobs); pytest transcript including the new fixture + canary tests; grep transcript that no other consumer assumes the list shape. IMPLEMENTED (cycle-8 batch G1, implement attempt ba17f3a99f20, worktree-uncommitted): parse now payload.get("jobs", payload) with list/dict shape guard, generated inventory carries a `# N jobs` count header, the sync aborts rc=1 loudly on missing/shapeless/empty jobs source, _fake_home gained a realistic dict-shaped jobs.json, both-shape + canary tests added (suite 68→77); acceptance (4) post-landing daily-run check still pending

### Anchor the gitleaks dist/ allowlist to a path boundary
- id: `rm-047` | track: reliability | priority: 78.0 | status: done in-tree, ship-gated (cycle-8 batch G2, implement attempt ba17f3a9: both entries anchored (^|/)web/dist/ + (^|/)dist/; dist/-exemption decision recorded — true build-output dirs stay exempt, dist-suffixed look-alikes scanned; locked by 1 static + 2 live-binary tests on gitleaks 8.28.0 and cloud-validated by leak-sentinel-self SUCCESS on CI's 8.30.1 (PR #13); no in-repo dist/ dir exists, so the change is behavior-neutral here and protective for fleet adopters)
- signals: cycle-8 assess re-proved LIVE 2026-09-26 (gitleaks 8.28.0 + this repo's config, /tmp A/B): identical planted token DETECTED at src/token.txt but SILENTLY EXEMPT at mydist/token.txt — gitleaks.toml:98 `'''dist/'''` is unanchored, so any *dist-suffixed directory is exempt (known since cycle-6 #10849, never tracked in this ledger); no dist/ dir exists in this repo today, so it is latent surface for fleet adopters of the shared config
...
- evidence: gitleaks A/B transcript post-change; pytest for the extended config test; the recorded dist/-exemption decision. IMPLEMENTED (cycle-8 batch G2, implement attempt ba17f3a99f20, worktree-uncommitted): BOTH entries anchored — gitleaks.toml now `'''(^|/)web/dist/'''` + `'''(^|/)dist/'''`; decision recorded as keep-exempt-for-true-build-dirs (xweb/dist/ is a true component and stays exempt by design); static + two live-binary tests lock the pair (planted token detected in mydist/, still exempt in dist/ and web/dist/); suite 68→77

### Daily enforcement of the deployed-artifact contract
- id: `rm-048` | track: reliability | priority: 88.0 | status: candidate
- signals: cycle-8 assess LIVE 2026-09-26 — the deployed trust chain went red and nothing noticed: MANIFEST pins (mtime 09-24 19:51) predate the cycle-7 landing (43d8097 at 21:22), verify_deployed_artifacts rc=1 with 3 delegate digest mismatches, undetected ≥36h; jobs.json scan + grep over deployed wrappers find NO cron and NO script that verifies the manifest (only the 08:30/08:40 sync jobs touch the repo) — rm-042's one-shot proof chain has no recurring enforcement, so drift-by-design (every landing) recurs silently
...
- evidence: jobs.json/crontab diff + first journaled run line; rc=0 transcript post re-pin; locking-test transcript. REPO HALF IMPLEMENTED (cycle-8 batch G3, implement attempt ba17f3a99f20, worktree-uncommitted): sync_data_branch.py now runs scripts/verify_deployed_artifacts.py as a subprocess BEFORE the data-branch switch (VERIFY_WRAPPER/VERIFY_MANIFEST overrides keep it hermetic; rc 0/1/2 + timeout/OS errors all caught, report-only) and journals every state change to control-plane/manifest-verify.log on the public data branch — deduped on state (CONTROL_PLANE_VERIFY_MAX_LINES, default 200), drift/clean/hermetic locking tests added, AGENTS.md documents the daily expectation; zero host edits (rides the existing 08:40 cron exec path, no jobs.json change); still pending: the commit-gate rm-042 re-pin + rc=0 proof and the first live journaled run

### Enable private vulnerability reporting on the public repos
- id: `rm-049` | track: customer-experience | priority: 84.0 | status: candidate — probe-first
- signals: cycle-8 research LIVE 2026-09-26 — gh api repos/{.github, magic-hermes, openai-embedding-proxy}/private-vulnerability-reporting → {"enabled":false} ×3 while SECURITY.md routes vulnerability reports to private GitHub advisories: the promised channel is OFF, so a researcher following our own policy hits a dead end; free on public repos, GET reachable = settable class (rm-043 pattern)
...
- evidence: PUT/GET transcripts ×3; SECURITY.md diff; revert-path capture from the probe

### Security-posture convergence: report-only drift lanes need an actor
- id: `rm-050` | track: reliability | priority: 74.0 | status: candidate (decision item)
- signals: cycle-8 LIVE 2026-09-26 — sync --dry-run prints `DRIFT security (report-only) … want enabled` for magic-hermes + openai-embedding-proxy (secret_scanning/push_protection/dependabot_security_updates ALL disabled, fresh GETs) and magic-context (dependabot) — rows have printed daily since cycle-3 with no convergence owner; dedicated endpoints corroborate (automated-security-fixes enabled:false ×5 probed); either the lane gains an --apply path (PATCH security_and_analysis / PUT automated-security-fixes on public repos, rm-016/rm-027 one-repo probe pattern) or posture is recorded manual-by-decision and the rows stop crying wolf
...
- evidence: decision text; dry-run transcript post-change (converged rows or revised semantics); probe transcripts for whichever path is chosen

### Code scanning default setup on the public code repos
- id: `rm-051` | track: reliability | priority: 55.0 | status: candidate (low)
- signals: cycle-8 research LIVE 2026-09-26 — repos/{3 public}/code-scanning/default-setup → {"state":"not-configured"} ×3; CodeQL default setup is free on public repos (single PUT) and needs no workflow maintenance; value bounded by the code volume in magic-hermes / openai-embedding-proxy (.github is mostly YAML)
...
- evidence: PUT/GET transcripts; first analysis run link; triage note

## Cycle-8 roadmap note (2026-09-26, post-assess, post-research)

Assess (attempt fbba2dfbf5b5, ce-review report-only inline, 6 findings — 1 P1 live
production + 4 P2 + 1 P3 — at 43d8097; 68/68 suite green; all 11 cycle-7 findings
verified fixed live) and research (attempt d505e4656651, ce-ideate inline, 7 survivors
ranked + 5 explicit rejections + 3 cleared surfaces, ~40 live probes 2026-09-26) fed
this addendum. STATUS FLIPS: rm-045 → done (landed cbf7bf1/43d8097, cycle-8
live-verified). NEW: rm-046 (P1 empty cron-inventory mirror + fixture + canary),
rm-047 (dist/ allowlist anchoring — tracked for the first time), rm-048 (daily
deployed-artifact enforcement, split from rm-042's rider), rm-049 (private
vulnerability reporting, probe-first), rm-050 (security-posture convergence
decision), rm-051 (code scanning default setup, low). EVIDENCE APPENDED: rm-042
(rider unexecuted + nothing schedules the check), rm-038 (5/43 callers ≈ 12%,
openai-embedding-proxy still zero), rm-035 (api 404 persists). CLEARED this cycle
(no item needed): default_workflow_permissions=read already set on all 5 probed
repos; gitleaks-action v3.0.0 / actions/checkout v7.0.1 / actionlint v1.7.12 /
zizmor v1.30.1 all current upstream (gitleaks v8.30.1 = CI pin; host binary 8.28.0
lags — host-side only); rulesets still 403-private / 200-empty-public (rm-012/rm-029
calculus unchanged); Renovate still 0 PRs (app not installed); corrections.yaml
verified truthful on sentinel coverage. REJECTED with rationale: settings-engine
migration (peribolos/Terraform — org-only or overkill for a personal account's 43
repos); org-level required workflows (org-only platform feature; codeo1io is a user);
version-bump items for already-current pins (would duplicate rm-039 keep-current).
Fleet note: 43 repos live via /user/repos (cycle-7 addendum recorded 46 — delta −3;
sync auto-covers, --dry-run RESULT OK).

## Cycle-8 compound note (2026-09-26, pre-review)

Batch G "inventory truth" implemented (attempt ba17f3a9, ce-work inline), validated,
and compounded here as REAL tree edits; test outcomes CONSUMED as recorded evidence
(#11119 lock honored — no test command invoked in compound): targeted (attempt
0476d557) py_compile 5/5 + 35/35 over the changed surfaces via the local focused
substitute (the named targeted_command's force-push/draft-PR mechanism is push/pr/ci
outside full-command authority); full (attempt e9192084) full_command VERBATIM →
ok:true, ephemeral PR #13 self-cleaned (leak-sentinel-self self-scan SUCCESS on CI
gitleaks 8.30.1 — the anchored allowlist cloud-validated), local corroboration 77/77
(README exact), digest validation:v1:79a1d3fc… unchanged across implement→full.
PROMOTIONS: rm-046 → done in-tree ship-gated (post-landing daily-run check is the
ship gate), rm-047 → done in-tree ship-gated. STILL OPEN WITH RIDERS: rm-048 host
half (commit-gate re-pin of the three delegate digests after canonical resets —
#11093 stale-by-design — then the first journaled verify line lands with the same
daily run); rm-022 down to its cron-wiring residual. LESSONS: L36–L39 appended to
docs/lessons.md (fix-claims cite the commit; production-shaped fixtures; anchored
allowlist paths; enforcement rides the paths that already execute).
NEXT CYCLE, in priority order (owner menu first): rm-049 private-vuln-reporting
PUT probe-first + rm-050 convergence decision + rm-051 code-scanning default setup
(account-level, owner-gated); rm-044 sentinel in-repo workflow + the carried CI-
pytest-gate candidate (the cloud gate still executes leak-scan only — no Python net,
#11117); rm-038 sentinel adoption at 43-repo scale (5/43 callers); rm-028 close-or-
drop (fleet read-converged); rm-016/rm-027 one-repo probes if rm-050 chooses apply;
housekeeping (owner-gated): two stale conductor/run-* validation refs on origin
(8606647750f7@cbf7bf1, 92e960046f14@c04a5498) are push-class deletions.

*End of cycle-8 addendum.*
