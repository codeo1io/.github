...
def test_comment_mention_joins_contract_fail_closed(tmp_path: Path) -> None:
    """Discovery is textual over the whole file: a deployed script that only
    MENTIONS one of our scripts/ paths in a comment still joins the pin
    contract. Fail-closed direction (demands extra pins, never fewer) —
    locked here so a future refactor cannot silently weaken it (review
    finding 5, cycle-7)."""
    wrapper, manifest = _make_deployed(tmp_path)
    noisy = wrapper.parent / "mentions-in-comment.sh"
    noisy.write_text(
        "#!/bin/sh\n"
        "# TODO: some day delegate via scripts/check_known_hosts.py like .github does\n"
        "exec bash /opt/other/thing.sh\n"
    )
    res = _run(wrapper, manifest)
    assert res.returncode == 1, "comment-only mention must still demand the pin"
    assert "mentions-in-comment.sh" in res.stderr
    assert "check_known_hosts.py" in res.stderr

    # pinning the wrapper AND the delegate clears the drift
    manifest.write_text(
        manifest.read_text()
        + f"{_sha(noisy)}  mentions-in-comment.sh\n"
        + f"{_sha(REAL_AUX_DELEGATE_2)}  check_known_hosts.py\n"
    )
    res = _run(wrapper, manifest)
    assert res.returncode == 0, res.stderr
...
def test_name_collision_demands_our_copy_fail_closed(tmp_path: Path) -> None:
    """A foreign wrapper whose same-named script collides with ours (their
    repo also has scripts/check_known_hosts.py) resolves into THIS repo from
    the verifier's view — the contract demands OUR copy pinned and digest-
    matched. Worst case is a spurious drift report, never a missed one
    (review finding 5, cycle-7)."""
    wrapper, manifest = _make_deployed(tmp_path)
    colliding = wrapper.parent / "other-repo-cron.sh"
    colliding.write_text(
        "#!/bin/sh\n"
        "cd /work/projects/other-repo\n"
        "exec python3 scripts/check_known_hosts.py\n"  # their copy — same name
    )
    res = _run(wrapper, manifest)
    assert res.returncode == 1, "name collision must demand OUR pinned copy"
    assert "other-repo-cron.sh" in res.stderr

    # satisfying the contract against OUR copy clears it
    manifest.write_text(
        manifest.read_text()
        + f"{_sha(colliding)}  other-repo-cron.sh\n"
        + f"{_sha(REAL_AUX_DELEGATE_2)}  check_known_hosts.py\n"
    )
    res = _run(wrapper, manifest)
    assert res.returncode == 0, res.stderr
